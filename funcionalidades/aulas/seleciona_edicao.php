	...
	<div id="ajuda">
		<div id="ajuda_meio">
			<div id="ajudante">
				...
				<div id="rel"><p id="balao">Nesse espaço se pode editar as aulas ja existentes. Basta selecionar a turma que deseja editar as aulas e clique em “Confirmar”.</p></div>
			</div>
		</div>
...
?>
		<div class="cor<?=alterna()?>">
			<ul class="margem_raquel">
				<li class="tabela">
					<div class="info">
						<a href="criar.php?turma=<?=$turma?>&aula_id=<?=$aula->getId()?>"><p class="nome"><b><?=$aula->getTitulo()?></b></p></a>
						<p class="data">Data: <?=$aula->getData()?></p>
					</div>
				</li>
				<li>
					<a class="no_underline" href="criar.php?turma=<?=$turma?>&aula_id=<?=$aula->getId()?>"><p class="texto_resposta"><?=$aula->getDesc()?></p></a>
					<div id="autor" class="criado_por">Criado por: <?=$nomecriador->resultado['usuario_nome']?></div>
					<a class="link_aula" href="criar.php?turma=<?=$turma?>&aula_id=<?=$aula->getId()?>"><b>Ver aula</b></a>
				</li>
			</ul>
		</div>
<?php

...
class Bg_barra{
	...
	function draw_bg(){
		...
		ImageLine(	$this->canvas,
					$this->borda,$this->size/2,
					$this->size-$this->borda,$this->size/2,
					$this->cor['white']);				//horizontal

		ImageLine(	$this->canvas,
					$this->borda,0,
					$this->borda,$this->size,
					$this->cor['white']);				//vertical

		ImageLine(	$this->canvas,
					0.9*$this->borda,0.5*$this->borda,
					1.4*$this->borda,0.5*$this->borda,
					$this->cor['white']);				//limite superior

		ImageLine(	$this->canvas,
					0.9*$this->borda,$this->size-0.5*$this->borda,
					1.4*$this->borda,$this->size-0.5*$this->borda,
...
$i+=1.5;/*-----------------	Imprime Barras	=>	confiança	--------------------------------*/
		if($conf!=0)
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2,
									($ini*$i+$lado)*$this->borda,			(1-$conf)*$this->size/2,
									$this->cor['red']);
		else
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2-$cima,
									($ini*$i+$lado)*$this->borda,			$this->size/2+$cima,
									$this->cor['red']);
$i+=1.5;/*-----------------	Imprime Barras	=>	esforço	------------------------------------*/
		if($esf!=0)
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2,
									($ini*$i+$lado)*$this->borda,			(1-$esf)*$this->size/2,
									$this->cor['green']);
		else
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2-$cima,
...
$i+=1.5;/*-----------------	Imprime Barras	=>	independência	-----------------------------*/
		if($indep!=0)
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2,
									($ini*$i+$lado)*$this->borda,		(1-$indep)*$this->size/2,
									$this->cor['blue']);
		else
			ImageFilledRectangle(	$this->canvas,
									$ini*$i*$this->borda,					$this->size/2-$cima,

...
function db_conecta () {
	...
	db_erro('N?o consegui me conectar no MySQL! '.mysql_error());
...
function db_faz ($query) {
	if ($q = mysql_query($query)) {
		...
		db_erro("db_faz('$query'): ".mysql_error());
		return FALSE;
	}
}
...
function db_busca($query) {
	if ($busca = mysql_query($query)) {
		...
		db_erro("db.inc.php: db_busca('$query'): ".mysql_error());
		return array();
	}
}
...
function db_conta($query) {
	if ($busca = mysql_query('SELECT COUNT(*) '.$query)) {
		...
		db_erro('db_conta(): '.mysql_error().' em SELECT COUNT(*) '.$query);
		return 0;
	}
}
...
function cataword($radical,$words){
	...
	if(!$flag){
		...
		$saida = array($word[$radical][$i]['qdr'],$word[$radical][$i]['sqd']);
	}
	return $saida;
}
function in_adv($str){
...
class Ppost{
	...
	public function inicia($data,$msg,$periodo){
		if(strpos($data,":")!==0){

...
var path = Array();				// path é um preview (SVG) da mão livre/ pincel
...
var INTERVALO_PARA_PONTOS = 20;	// intervalo de tempo em que a posição do mouse será guardada (para fazer o risco)
...
function coletaPontos(){
	...
	if (index_ultimo_elemento == -1){ // array vazio. simplesmente aceita o novo ponto. (ponto inicial do path)
		path.push(["M", cursor.x, cursor.y]);	// para entender o que ocorre aqui vale dar uma procurada no google sobre a documentação do SVG
		...
		if (mouse.borracha){		// verifica o tipo de ferramenta, pois cada uma tem um preview diferente
			...
			mao_livre_preview.push(tela_svg.path(path).attr({stroke: linha.cor, "stroke-width": linha.largura, "stroke-linecap": "rounded"})); // lápis (mão livre)
		}
	}else{
...
// senão, a bezier é construída
			...
			path.push(["L", cursor.x, cursor.y]); // faz linha reta para o último ponto

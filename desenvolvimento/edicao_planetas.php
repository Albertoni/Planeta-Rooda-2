<?php
	...
	$tiposTerrenos = $_POST['tiposTerrenos'];
	$acesso = $_POST['acesso'];
	$edicao = $_POST['edicao'];
	...
	$tiposTerrenosParaEditar = explode(",", $tiposTerrenos);
	$ID_TERRENO_CRIADO = -1;
	for($i = 0; $i<count($idsTerrenosParaEditar); $i++){
		if($idsTerrenosParaEditar[$i] != $ID_TERRENO_CRIADO){
			$updateTerrenos->solicitar("UPDATE terrenos SET terreno_grupo_id=$identificacao,terreno_nome='$nomesTerrenosParaEditar[$i]', terreno_solo='$tiposTerrenosParaEditar[$i]', terreno_permissao_edicao='$edicaoAtual' WHERE terreno_id='$idsTerrenosParaEditar[$i]'");
		} else {
			...
			$updateTerrenos->solicitar("INSERT INTO terrenos (terreno_grupo_id, terreno_nome, terreno_solo, terreno_permissao_edicao, terreno_indice, chat_id) VALUES ($identificacao, '$nomesTerrenosParaEditar[$i]', '$tiposTerrenosParaEditar[$i]', '$edicaoAtual', $indice_terreno_no_planeta, $idChat)");
			$pesquisaIdTerrenoInserido = new conexao();
			$pesquisaIdTerrenoInserido->solicitar("SELECT * FROM terrenos WHERE terreno_nome='$nomesTerrenosParaEditar[$i]' AND terreno_solo='$tiposTerrenosParaEditar[$i]'");
			$idsTerrenosParaEditar[$i] = $pesquisaIdTerrenoInserido->resultado['terreno_id'];
		}
	}
	...
	if($nome != null){
		$pesquisaEdicaoPlanetaSQL.="Nome = '$nome',";
	}
	else{
		$pesquisaEdicaoPlanetaSQL.="Nome = '$nomeAtual',";
	}
	if($tipo != null){
		$pesquisaEdicaoPlanetaSQL.="Tipo = $tipo,";
	}
	else{
		$pesquisaEdicaoPlanetaSQL.="Tipo = $tipoAtual,";
	}
	if($idsTerrenos != null){
		$pesquisaEdicaoPlanetaSQL.="Terrenos = '$idsTerrenos',";
	}
	else{
		$pesquisaEdicaoPlanetaSQL.="Terrenos = '$idsTerrenosAtual',";
	}
	if($dono != null){
		...
		$pesquisaEdicaoPlanetaSQL.="IdResponsavel = $idDonoAtual,";
	}
	if($acesso != null){
		$pesquisaEdicaoPlanetaSQL.="acesso = '$acesso',";
	}
	else{
		$pesquisaEdicaoPlanetaSQL.="acesso = '$acessoAtual',";
	}
	if($edicao != null){
		$pesquisaEdicaoPlanetaSQL.="edicao = '$edicao'";
	}
	else{
		$pesquisaEdicaoPlanetaSQL.="edicao = '$edicaoAtual'";
	}

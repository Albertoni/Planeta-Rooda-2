...
header('Content-Type: text/html; charset=utf-8');
	...
	*/
	$id_planeta_para_deletar = $_POST['identificacao'];
	$id_usuario_online = $_SESSION['SS_usuario_id'];
	
	/*
	* Deleta o planeta cujo id é passado como parâmetro, desde que o usuário online tenha permissão para isto.
	* @param planeta_id_param Id do planeta que será deletado.
	...
	*/
	function deletarPlaneta($planeta_id_param){
		...
		if($statusBusca == $SUCESSO){
			$conexao = new conexao();
			$conexao->solicitar("DELETE FROM Planetas
								 WHERE Id = $planeta_id_param");
			if($conexao->erro != ''){
				$statusBusca = $ERRO_CONEXAO_BD;	
			}
			$conexao->solicitar("DELETE FROM $tabela_terrenos
								 WHERE terreno_grupo_id = $planeta_id_param");
		}
	...
	$resultadoOperacao = deletarPlaneta($id_planeta_para_deletar);
